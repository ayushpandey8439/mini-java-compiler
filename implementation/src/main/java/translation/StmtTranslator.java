...
public class StmtTranslator implements NQJStatement.MatcherVoid {
    ...
    public StmtTranslator(Translator translator, TranslationTable translationTable) {
        this.tr = translator;
    ...
    public void case_VarDecl(NQJVarDecl s) {
        ...
        BasicBlockList currentBasicBlockList = translationTable.getCurrentBasicBlockList();
        // no code, space is allocated at beginning of method
        TemporaryVar varDecl = Ast.TemporaryVar("var_" + s.getName());
        currentBasicBlockList.get(0).addFront(Ast.Alloca(varDecl, tr.translateType(s.getType())));
        translationTable.addVariableToScope("var_" + s.getName(), varDecl);
        //Initialise the variable with its default value
        varDecl.calculateType()
                .match(new InitialisationTranslator(Ast.VarRef(varDecl), false, false,
                        currentBlock));
        //tr.initialiseFields(Ast.VarRef(varDecl), false, currentBlock);
    ...
    public void case_StmtAssign(NQJStmtAssign s) {
        ...
        //translationTable.addVariableToScope();
